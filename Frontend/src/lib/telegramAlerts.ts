...
const BOT_TOKEN = import.meta.env.VITE_TELEGRAM_BOT_TOKEN || '8889927818:AAETEXfIph1TZxJgK5BaLtawKYhYRXIIn1M';
const CHAT_ID = import.meta.env.VITE_TELEGRAM_CHAT_ID || '-1003911645931';
const RESERVATION_THREAD_ID = Number(import.meta.env.VITE_TELEGRAM_RESERVATION_THREAD_ID || 2);
const FEEDBACK_THREAD_ID = Number(import.meta.env.VITE_TELEGRAM_FEEDBACK_THREAD_ID || 4);

export async function sendTelegramMessage(text: string, messageThreadId?: number): Promise<boolean> {
  try {
...
export async function sendFrontendReservationAlert(reservation: any): Promise<boolean> {
  ...
  if (!branch) {
    ...
    else if (reservation.branch_id === 2) branch = 'One More Restaurant Boeung Kak';
  ...
  let preorderText = '';
  const items = reservation.preordered_items || reservation.preorder_items || [];
...
export async function sendFrontendFeedbackAlert(feedback: any): Promise<boolean> {
  const nameDisplay = (feedback.name || feedback.customer_name)?.trim() ? (feedback.name || feedback.customer_name) : 'Anonymous Guest';
